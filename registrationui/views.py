...
def index(request):
    if request.method == 'POST':
	    ...
	    print 'Form data:', username, first_name, last_name, email, password

	    con = ldap.initialize(settings.LDAP_URL) 
	    print 'Init:' + str(con.bind(settings.LDAP_ADMIN_CN, settings.LDAP_PASSWORD, ldap.AUTH_SIMPLE))
...
# Create the new user record:
	    ...
	    print 'Add: ' + str(con.add_s('uid=' + str(username) + ',' + settings.LDAP_USERS_CN, add_record))
	    con.unbind()
        ...
        form = NewUserForm()
    return render(request, 'registrationui/index.html', { 'form': form } )

def thanks(request):
    return render(request, 'registrationui/thanks.html' )
